...
import { setRoomsFlagAction, getAll } from "../Actions";
...
class BookingSuggestions extends React.Component {
  ...
  render() {
    ...
    let flag = true;
    let fromToDate = [];
    console.log("BookingSuggestions : gapArrayData = >", this.props.gapArrayData);
    ...
    returnContent = this.props.gapArrayData.gapArray.map((gapRoom, i) => {
      ...
      if (gapRoom.gap.length > 0) {
        suggestionDateList = fromToDate.map((gap, i) => {
        ...
        return <div>{suggestionDateList}</div>;
      }
    });
    return <div class="suggestions">{returnContent}</div>;
  }
}
