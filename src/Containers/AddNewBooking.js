...
class AddNewBooking extends React.Component {
	...
	handleSubmit(e) {
		...
		if (this.props.showRoomFlag.addOrUpdate === "UPDATE") {
			...
			if (empArrLen === parseInt(capacity, 10)) {
				this.setState({
					alertClassName: "row showAlert",
					message: "Selected ROOM Is FULL for Selected Dates, Change your dates or Room"
				});
				document.getElementById("message").focus();
			} else {
	...
	render() {
		...
		return ( <
				...
				showBorder className = "cal"
				onChange = {
					date => this.handle_checkOut(date)
				}
				placeholder = {
					"Select a date"
				}
